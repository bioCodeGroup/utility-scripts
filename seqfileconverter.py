...
def get_args():
    ...
    parser.add_argument('-d', '--discard-file',
                        metavar='DISCARDFILE', help='file containing sequences\
                        discarded by QC')
    parser.add_argument('-t', '--file-type',
                        metavar='FILETYPE', help='type of input and output files\
                        qf=qseq2fastq fq=fastq2qseq sf=sam2fastq fs=fastq2qseq')
    ...
    try:
        with open(qseq_file) as f:
            ...
            for line in f:
                ...
                if len(fields) == 11 and fields[10] == '1' and \
                len(fields[8]) == len(fields[9]) and val.isfloat(fields[4]) is True and \
                val.isfloat(fields[5]) is True and val.isdna(fields[8]) is True:
...
def fastq_to_qseq(fastq_file, qseq_file, metadata_file):
    """
    ...
    try:
        ...
        with open(fastq_file) as f:
            while True:
                ...
                if len(seq) == len(qual) and val.isfloat(header[4]) is True and \
                val.isfloat(header[5]) is True and val.isdna(seq) is True:
...
def main():
    ...
    """
    #Determine file type with -t input and use the according file converter
    args = get_args()
    if args.file_type == 'qf':
        ...
        fastq_to_qseq(args.input_file, args.output_file, args.metadata_file)
        ...
        print('Your file type input does not match existing types, try again \
        using qf, fq, sf, or fs. ')
